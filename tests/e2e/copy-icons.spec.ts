...
Note: Inline code like \`npm install\` doesn't need a copy button.`,
  ...
  test('04 - Hover over first code block to reveal copy button', async () => {
    ...
    const codeBlockWrappers = await window.evaluate(() => {
      const wrappers = Array.from(document.querySelectorAll('.relative.group')).filter((w) =>
        w.querySelector('pre')
      );
    ...
    await window.evaluate(() => {
      const wrappers = Array.from(document.querySelectorAll('.relative.group')).filter(
        (w): w is HTMLElement => w.querySelector('pre') !== null
      );
      if (wrappers[0]) {
