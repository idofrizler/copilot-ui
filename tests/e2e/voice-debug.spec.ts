...
test.describe.skip('Voice Input Debug', () => {
  ...
  test('should click mic button and capture any errors', async () => {
    ...
    const micButton = window
      ...
      .or(window.locator('[data-testid="mic-button"]'))
      .or(window.locator('button').filter({ has: window.locator('svg[class*="microphone" i]') }));
    ...
    if (hasMicButton) {
      ...
      const errorElement = window
        .locator('text="Speech error"')
        .or(window.locator('text="network"'))
        .or(window.locator('[class*="error"]'));

      const hasErrorUI = await errorElement
        .first()
        .isVisible()
        .catch(() => false);
      ...
      const voicePanelError = await window
        .locator('.text-copilot-error')
        .first()
        .textContent()
        .catch(() => null);
      if (voicePanelError) {
    ...
    if (result.errors.length > 0) {
      result.errors.forEach((err, i) => {
        ...
        if (err.includes('network')) {
          console.log(
            '  → Network error: Speech recognition requires internet connection to Google servers'
          );
          console.log(
            '  → This is expected in Electron if offline or if Google Speech API is blocked'
          );
  ...
  test('should dump all console logs at end', async () => {
    ...
    const speechErrors = consoleErrors.filter(
      (e) =>
