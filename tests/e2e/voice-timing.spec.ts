...
test('Voice initialization timing and errors', async () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (i % 5 === 0) {
      console.log(
        `[${Date.now() - startTime}ms] Still loading... (hasLoading: ${status.hasLoading})`
      );
    }
  }
  ...
  // Final summary
  console.log('\n=== TIMING SUMMARY ===');
  const voskLogs = logs.filter(
    (l) => l.text.toLowerCase().includes('vosk') || l.text.toLowerCase().includes('model')
  );
