...
test.describe('Mark Session as Unread Feature', () => {
  ...
  test('05 - Add Note opens modal', async () => {
    ...
    await expect(window.locator('text=/Add Review Note|Edit Review Note/')).toBeVisible({ timeout: 3000 });
  ...
  test('10 - Dismiss note banner', async () => {
    ...
    const dismissButton = window.locator('.bg-blue-500\\/10 button, [class*="bg-blue"] button').first();
