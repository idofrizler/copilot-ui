...
test.describe.skip('Voice Input Feature', () => {
  test('should have microphone button visible', async () => {
    ...
    const micButton = window.locator('[data-testid="mic-button"]').or(
      window.locator('button').filter({ has: window.locator('svg') }).filter({ hasText: /mic/i })
    ).or(
      window.locator('button[title*="voice" i], button[title*="mic" i], button[aria-label*="voice" i], button[aria-label*="mic" i]')
    )
  ...
  test('should display VoiceKeywordsPanel when voice is supported', async () => {
    ...
    if (hasVoicePanel) {
      ...
      const copilotKeyword = window.locator('text="copilot"').or(window.locator(':text("copilot")'))
  ...
  test('should show status indicator in voice panel', async () => {
    ...
    console.log('Voice status visible:', hasAnyStatus, { hasReadyStatus, hasRecordingStatus, hasSpeakingStatus })
  })

  test('should have mute/unmute toggle button', async () => {
    // Look for mute button in voice panel
    const muteButton = window.locator('button:has-text("🔊")').or(
      window.locator('button:has-text("🔇")')
    ).or(
      window.locator('button[title*="mute" i], button[title*="Mute" i]')
    )
    
    const hasMuteButton = await muteButton.first().isVisible().catch(() => false)
  ...
  test('should show usage hint', async () => {
    ...
    const usageHint = window.locator('text="Click mic button to record"').or(
      window.locator(':text("Speech is transcribed")')
    )
    
    const hasHint = await usageHint.first().isVisible().catch(() => false)
