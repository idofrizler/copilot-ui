...
export function parseSkillFrontmatter(content: string): { name?: string; description?: string; license?: string } {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        errors.push(`Failed to read SKILL.md at ${skillMdPath}: ${err instanceof Error ? err.message : String(err)}`)
      }
    }
  } catch (err) {
    errors.push(`Failed to scan skills directory ${basePath}: ${err instanceof Error ? err.message : String(err)}`)
  }
