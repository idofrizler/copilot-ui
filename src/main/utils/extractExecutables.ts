...
const FLAGS_WITH_ARGUMENTS = new Set([
  // HTTP/curl flags
  '-X',
  '--request', // HTTP method (GET, POST, PUT, etc.)
  '-H',
  '--header', // HTTP header
  '-d',
  '--data', // POST data
...
export function containsDestructiveCommand(command: string): boolean {
  ...
  if (
    commandLower.includes('xargs rm') ||
...
export function extractFilesToDelete(command: string): string[] {
  ...
  for (const segment of segments) {
    ...
    const rmMatch = trimmed.match(
      /(?:^|(?:sudo|env|nohup|nice|time|command)\s+)*(rm|rmdir|unlink|shred)(?:\s|$)(.*)/
    );
    if (!rmMatch) continue;
