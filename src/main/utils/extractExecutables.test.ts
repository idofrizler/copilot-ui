...
import {
  ...
  isDestructiveExecutable,
  extractFilesToDelete,
} from './extractExecutables';
...
describe('extractExecutables', () => {
  ...
  describe('git and subcommand handling', () => {
    ...
    it('extracts multiple git commands', () => {
      expect(extractExecutables('git add . && git commit -m "test"')).toEqual([
        'git add',
        'git commit',
      ]);
    });
...
for i in 1 2 3; do 
  ...
  describe('destructive command detection - Issue #65', () => {
    ...
    describe('containsDestructiveCommand', () => {
      ...
      it('detects find with -exec /bin/rm', () => {
        expect(containsDestructiveCommand('find /tmp -name "*.tmp" -exec /bin/rm {} \\;')).toBe(
          true
        );
      });
      ...
      it('detects the exact command from Issue #65', () => {
        const command =
          "find . -maxdepth 1 -type d -name '*-*-*-*-*' ! -name '1f8034d3-...' -exec rm -rf {} +";
  ...
  describe('extractFilesToDelete - Issue #101', () => {
    ...
    it('handles chained rm commands', () => {
      expect(extractFilesToDelete('rm file1.txt && rm file2.txt')).toEqual([
        'file1.txt',
        'file2.txt',
      ]);
    });
    ...
    it('handles paths with special characters', () => {
      expect(extractFilesToDelete('rm /tmp/test-file_v1.2.3.txt')).toEqual([
        '/tmp/test-file_v1.2.3.txt',
      ]);
    });
