  ...
  try {
    ...
    const resumePromises = openSessions.map(async (storedSession) => {
      ...
      try {
        ...
        // Store in sessions map first so the guard in registerSessionEventForwarding works
        ...
        // Register event forwarding (with guard to prevent duplicates on model change)
        registerSessionEventForwarding(sessionId, session);
...
async function initCopilot(): Promise<void> {
  try {
    ...
    // Wait for early resumption to complete before checking what was already resumed
    // This prevents a race condition where we'd resume the same session twice
    if (earlyResumptionPromise) {
      await earlyResumptionPromise;
    }
