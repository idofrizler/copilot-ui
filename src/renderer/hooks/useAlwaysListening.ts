...
export function useAlwaysListening(options: UseAlwaysListeningOptions) {
  const { enabled, isRecording, onWakeWordDetected, onStopWordDetected, onAbortWordDetected } =
    options;
  ...
  // Reset error when disabled (allows retry on re-enable)
  useEffect(() => {
    if (!enabled) {
      setError(null);
  ...
  useEffect(() => {
    // Don't retry if we already have an error - prevents infinite loop
    if (!enabled || !isSupported || isModelLoaded || isLoading || error) return;
    ...
    loadModel();
  }, [enabled, isSupported, isModelLoaded, isLoading, error]);
